...
resnet_v2.default_image_size = 224 # TODO what should this actually be
...
#     image_size = resnet_v2_200.default_image_size
#     img1 = imread('poodle.png', mode='RGB')
#     img1 = imresize(img1, (image_size, image_size))
#     inputs = np.array([img1.astype(np.float32)])
# 
#     net, end_points = resnet_v2_200(inputs, num_classes=1000, is_training=False)
#     
#     probs = tf.nn.softmax(net)
#     prob_values, indices = tf.nn.top_k(probs, k=5)
#     
#     with tf.Session() as sess:
#       init = tf.global_variables_initializer()
#       sess.run(init)
#       probs = probs.eval(session=sess)
# 
#     preds = (np.squeeze(np.argsort(probs))[::-1])[0:5]
# 
#     for p in preds:
#       print(class_names[p], probs[0, p])

  image_size = resnet_v2_152.default_image_size
  ...
  with tf.Session() as sess:
    network = resnet(imgs, sess)
    processed_images = resnet.preprocess(images)
  ...
  for prob in probabilities:
    print('\n')
    settings.get_class_labels(range(im_ind, im_ind + 1))
    im_ind += 1
    inds = np.argsort(prob)[::-1]
    print(inds[0:5])
    for j in range(5):
      print(class_names[inds[j]], prob[inds[j]])
