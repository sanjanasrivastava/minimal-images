...
def create_confidence_map():
    ...
    # im = im.resize((10, 10))	# for debugging
    width, height = im.size

    # Get dimensions of each necessary crop
    if CROP_TYPE == 'proportional':
        crop_size = int(((PROPORTION * height) / 2) * 2 + 1)     # Make sure its odd
    else:
        crop_size = CONSTANT
    center_offset = int(crop_size / 2)
    crop_x1s = range(width - crop_size + 1)
    crop_y1s = range(height - crop_size + 1)
    crop_dims = itertools.product(crop_x1s, crop_y1s)

    # Build map
    C = np.zeros((height - (center_offset * 2) + 1, width - (center_offset * 2) + 1))   # the math of these plus ones kind of escapes me, but I think it's because you get crop_size + 1 crops

    sess = tf.Session()
    imgs = tf.placeholder(tf.float32, [None, 224, 224, 3])
    vgg = vgg16(imgs, 'vgg16_weights.npz', sess)

    for x1, y1 in crop_dims:

        print('testing', x1, y1)

        # Run the test, get the confidence for the true label, place it in the map
        cropped = im.crop((x1, y1, x1 + crop_size, y1 + crop_size))
        img1 = imresize(cropped, (224, 224))
        prob = sess.run(vgg.probs, feed_dict={vgg.imgs: [img1]})[0]
        confidence = prob[TRUE_CLASS]
        r, c = y1, x1
        C[r, c] = confidence

    print(C)
    # Visualize map
    confidence_map = Image.fromarray(255. * C)
    # confidence_map.show()
    if confidence_map.mode != 'RGB':
        confidence_map = confidence_map.convert('RGB')
    confidence_map.save(PATH_TO_DATA + CONFIDENCE_MAP_FOLDER + image_tag + '_cm.JPEG', 'JPEG')
    print(image_tag)
